...
Lung_cancer_data_set <- read_delim("BiolDS-OT-SN-Report.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
# make column Run
data <- Lung_cancer_data_set %>% 
  unite(Run, R.Condition, R.Replicate, sep=".", remove = F) 
# if PG.ProteinAccessions is used as protein ID
if("PG.ProteinAccessions" %in% colnames(data)){
  data$IDPicker.InferenceId <- data$PG.ProteinAccessions
}
