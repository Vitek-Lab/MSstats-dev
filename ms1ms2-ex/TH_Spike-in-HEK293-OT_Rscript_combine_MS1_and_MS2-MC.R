...
## change the working directory to your local path
setwd("C:/Dropbox (Biognosys)/Biognosys_TH/2018 analyses/GSDS/")
...
GSDS_SN_report_final_noMissingValues_V11 <- read_delim("Spike-in-HEK293-OT-SN-Report.txt", "\t", escape_double = FALSE, trim_ws = TRUE)
# make column Run
data <- GSDS_SN_report_final_noMissingValues_V11 %>% 
  unite(Run, R.Condition, R.Replicate, sep=".", remove = F) %>%
  filter(R.Condition != "SGSDSsample8") # remove sample 8
...
write.csv(info, 'Spike-in-HEK293-OT-annotation.csv')
...
rm.proteins <- c("P07724", "Q921I1", "P02768", "P02787")
data <- data %>% 
...
nrow(filtered_data_long) # 1198420
...
nrow(filtered_data_long) # 1116486
